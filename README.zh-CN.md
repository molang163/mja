...
> **🎯 适用人群：**
> 既想要系统绝对稳定，又想安全使用海量 AUR 软件的 Manjaro 桌面用户。特别适合喜欢用 GUI 图形界面来管理容器化应用的人。
> 
> **🛑 不适用人群：**
> 非 Manjaro 发行版的用户；或正在寻找通用型、跨发行版包管理工具的用户。
...
**⚠️ 警告：** 在 Manjaro 系统中，**严禁使用 `pip install PySide6`**，这会破坏系统的 externally-managed 环境。请务必使用系统包管理器安装依赖：
